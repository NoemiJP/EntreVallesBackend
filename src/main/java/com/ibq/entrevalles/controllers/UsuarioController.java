...
public class UsuarioController {
	...
	@PostMapping("/login")
	public @ResponseBody ResponseEntity<Usuario> login(@RequestBody LoginRequest request) {
		Usuario usuario = this.usuarioRepository.findByEmailAndContrasenya(request.getEmail(),
	...
	public @ResponseBody ResponseEntity<Usuario> registro(@RequestBody Usuario usuario) {
		return ResponseEntity.ok(this.usuarioRepository.save(usuario));
	}

	@GetMapping("/login/{id}")
	public @ResponseBody ResponseEntity<Usuario> loginById(@PathVariable Long id) {
	...
	public ResponseEntity<Usuario> establecer(@RequestBody NuevaContrasenya usuario) {
		...
		if (usuarioExistente != null) {
			...
			try {
				sendSimpleMessage(usuario.getEmail(), "NUEVA CONTRASEÑA ENTREVALLES Y MONTAÑAS",
